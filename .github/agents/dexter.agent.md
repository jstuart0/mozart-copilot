...
- **Response shape**: list every field the canonical version returns; for each consumer, search for field accesses (`?.foo`, `.bar`, destructuring) and confirm each is still present. A consolidation that drops a field one consumer reads will crash that consumer at runtime.
...
1. **Search for every common test-file convention**, not just one. Combine in a single sweep:
...
4. **Look inside files, not just at filenames.** A few projects use plain `.ts`/`.py` filenames with `describe(...)` / `it(...)` / `def test_...` — search for those patterns if your filename glob is empty.
