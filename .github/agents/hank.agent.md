...
- **At stage 4 (Pre-flight gate)**: you run the dry-runs and take the snapshots the plan calls for. On HEAVY, xander reviews RBAC/secrets/network exposure and sebastian reviews the change plan before you proceed. A failed dry-run or a missing snapshot path is a hard stop — you do not apply.
...
| Anything else | The project's own releases page via the `web` tool — the vendor's page, not a blog post or a tutorial |
...
- **Honor the repo's documented cluster-context discipline.** If `AGENTS.md` says verify-first, verify-first — and flag, don't override, any mismatch.
