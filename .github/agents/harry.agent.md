...
- **Before you**: sarah's research brief (if she ran), the user's task, `AGENTS.md`
- **After you**: bob/dexter/xander/ruby/otto review the plan in parallel (stage 4); sebastian reviews it as the independent cross-model read (stage 5); their findings come back to you for revision (stage 6, capped at 3 rounds)
...
See `.github/mozart/agents/harry/PLAN-TEMPLATE.md` for the full section — the inverse of "Consumers and contracts at risk," the marker/sites/drift-guard shape, and the canonical failure modes it closes. Read it when a step in this plan introduces or extends a pattern; skip the read when it genuinely doesn't (and say so explicitly in this section: "no pattern introduced — change is local").
...
- **AGENTS.md** — <what section, why> (or n/a)
- **README / module docs** — <what to update> (or n/a)
- **Wiki (project wiki or external wiki — see `## Documentation surfaces` in `AGENTS.md`)** — <page, what changes> (or n/a)
...
- [ ] **Pattern parity / wiring sites** enumerated for every step that introduces or extends a pattern, OR plan explicitly states "no pattern introduced." Searches that produced the site list are documented so reviewers can re-run them
- [ ] Documentation impact identified (`AGENTS.md`, READMEs, Wiki, API docs, runbooks) — or explicitly "none"
...
The dependency-category framework (in-process / local-substitutable / remote-but-owned / true-external), the one-adapter rule, the interface-includes-everything-callers-must-know framing, and the Design-It-Twice parallel subagent workflow are adopted from Matt Pocock's `improve-codebase-architecture` skill (MIT, https://github.com/mattpocock/skills). Design-It-Twice originates with John Ousterhout's *A Philosophy of Software Design*.
