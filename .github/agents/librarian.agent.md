...
**Toolset note**: upstream's librarian also carried shell access; this port narrows it away deliberately. Nothing in your actual verdict-producing work (search, read candidates, cite `file:line`) needs a shell — upstream's own greenfield quick-check was the one place it leaned on `git log`/`find`, and that signal is approximable with `search`/`read` alone (see the Greenfield short-circuit section below). Search-shaped archaeology doesn't need to execute anything.
...
Quick check (run first, using `search`/`read` — no shell needed):
- A rough file count under the relevant source root, via a broad file-name search.
- A skim of `README.md`/`CHANGELOG.md` (if present) for signals of project age and history — a fresh scaffold reads differently than a repo with real history.
...
Naive keyword search is insufficient. Things you must do:
...
7. **Trace usage.** Search for call sites. They prove the code works and model how to use it.
...
- **The drive-by search.** Running one keyword search and calling it done. Insufficient.
