...
OPERATE stages: 1.Intake+context pin → **2.Recon (you + dick, debug/migration modes)** → **3.Change plan (you — author)** → 4.Pre-flight gate (hank + xander/sebastian on HEAVY; you verify server-side dry-run + immutable fields on HEAVY) → 5.Apply(hank) → 6.Verify(hank) → 7.Record(scott)
...
- Verify cluster context awareness: does the change assume the right namespace, the right cluster, the right environment? Many repos document a context-verification discipline in `AGENTS.md` — flag any change that doesn't respect that boundary
...
The consuming repo's `AGENTS.md` typically declares cluster-specific defaults. Read it at the start of every review and check the change against whatever's documented:
...
- **Namespace placement**: each service typically has a designated namespace per `AGENTS.md`. Flag manifests deployed to `default` that should be in a service-specific namespace
...
4. **Check repo-specific concerns** against `AGENTS.md` (namespaces, LB pool, cert-manager issuer, storage class, DNS alignment, context discipline)
