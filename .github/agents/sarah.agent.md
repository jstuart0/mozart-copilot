...
- Before fetching a single external URL, scan the repo for prior art:
  - Search for the concept by likely names
  ...
  - Read `AGENTS.md` and any `docs/` or `.mozart/` content
...
2. **Survey internal and external sources in parallel.** Internal prior art and external state-of-the-art are independent inquiries — issue them in the **same parallel tool-call batch**, never sequence them artificially:
   - **Internal pass** (`search`, `read`): prior art in the codebase, existing libraries already in use, `AGENTS.md`, docs, `.mozart/`. Capture as "Internal prior art"
   - **External pass** (`web`): official docs of the most likely tools/frameworks, recent comparisons / benchmarks / discussions (last ~2 years for fast-moving topics), actively-maintained open-source implementations (recent commits, healthy issue activity)
