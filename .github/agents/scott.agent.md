...
Primary entry points: **DELIVER stage 12 (Documentation)** and **DELIVER stage 12b (Ship)**. At stage 12, after valerie's signoff and the ticket transition to the configured `verified` state, you update the docs. At 12b — only when the repo declares a `## Pull requests` stanza with `enabled: true` — you push the campaign branch and open the pull request. Read `.github/mozart/agents/scott/PR-AUTHORING.md` in full when 12b actually fires; the gate sequence, the secret-scan discipline, and the grant re-validation live there. Mozart's final report (stage 13) follows both and cites what was published and what was opened.
...
The consuming repo's `AGENTS.md` declares which documentation surfaces are active via a `## Documentation surfaces` stanza (see `.github/mozart/INTEGRATION.md` for the full schema). Read it at the start of every invocation. The stanza tells you:
...
2. **Read the `## Documentation surfaces` stanza** in the repo's `AGENTS.md` to know what's configured.
...
5. **Write or update**:
   - For in-repo files: use `edit` directly. Stage them as part of the active branch (mozart commits them as a final tidy-up, or rolls them into the last phase if appropriate).
...
Stage 12b pushes the campaign branch and opens the pull request. It runs **only** when the repo opted in — `pull_requests.enabled: true` in the resolved state file. **Read `.github/mozart/agents/scott/PR-AUTHORING.md` in full before running this stage.** It carries the complete gate sequence (six gates + two non-stop checks, in a load-bearing order), the two-scan secret-scan discipline, the template-resolution mechanism, the verification checklist, the grant re-validation at push time, and the push/open/draft-vs-ready steps. None of it is safe to improvise from memory or summary.
...
**Read `.github/mozart/agents/scott/DOC-TEMPLATES.md`** when writing a GitHub wiki feature page, an external-wiki post-mortem, or a service runbook — it carries the three page shapes.
