...
Mozart is pluggable for five surfaces: **ticketing**, **documentation**, **code retrieval**, **worktrees**, and **pull requests**. You configure them by adding stanzas to your repo's `AGENTS.md`. Mozart and its specialists read those stanzas at intake; if a stanza is missing, the corresponding behavior is skipped or falls back to a sensible default.

This file is the contract. Copy the appropriate stanza into your repo's `AGENTS.md`, fill in the values, and the bundle adapts.
...
Every code-reading agent carries a **"Code retrieval"** gate. The gate is product-neutral: if the consuming repo declares a code-aware retrieval tool, agents route source-code reads through it (symbol search, file outlines, symbol-source fetch, reference/call-hierarchy lookups) ahead of a broad `search`/`read`, falling back to plain retrieval only for non-code files, byte-exact pre-`edit` reads, tiny known-offset reads, or plan-mandated searches. Code-aware indexes typically cut retrieval token usage by 80-95% on source.

**If you don't configure one, nothing changes** — agents use plain `read`/`search`.
...
1. **Declare the tool** in your repo's `AGENTS.md` so agents know it exists and is authoritative:
   ...
   Prefer it over a plain search/read for source files. Resolve the repo at
   session start; if indexed, it is the first-choice retrieval tool.
   ```

2. **Grant the tool to the agents.** A subagent can only call tools in its `tools:` frontmatter allow-list. The shipped agents intentionally do **not** list any MCP server (the bundle stays tool-agnostic). To wire one in, append its tool pattern to the `tools:` line of each code-reading agent in your install — e.g. `<server>/*`. Agents whose `tools:` omits the MCP server can't use it regardless of the `AGENTS.md` declaration.

**Reference implementation:** an MCP server exposing symbol-level operations — `resolve_repo`, `search_symbols`, `get_file_outline`, `get_symbol_source`, `find_references`, `get_call_hierarchy`, `find_importers`, `get_dependency_graph` — is the shape this gate is written against. Wiring one in means adding `<server-name>/*` to each agent's `tools:` line and the `## Code retrieval` stanza above. Any LSP, IDE symbol index, or AST-backed MCP server that offers equivalent operations works the same way. No specific server is named here — this repo stays product-neutral by design.
...
**Artifacts never move into the worktree.** `.mozart/` stays at the root of the canonical checkout regardless of this stanza — that's what makes a single listing of `.mozart/plans/active/*.state.md` a complete view of in-flight campaigns.
...
Switching from one ticketing system to another (e.g., Plane → Linear) is just an `AGENTS.md` edit. Mozart re-resolves at the next intake. In-flight campaigns keep their original ticket; new campaigns use the new system.
