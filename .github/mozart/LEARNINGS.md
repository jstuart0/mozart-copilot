...
4. **Generalizes.** It's not specific to one repo's conventions, one customer's IdP, one team's preferences. Those go in the project's `AGENTS.md`, not your persona.

## What does NOT belong in field notes

- **Project-specific conventions.** "In `os-project-athena`, services live in `src/`" → repo `AGENTS.md`.
...
- When the pattern is project-specific → write to the project's `AGENTS.md` instead.
...
## How agents without an `edit` grant append

Most agents have either `edit` or `execute` (heredoc append via the shell). For those with only `execute`:

```bash
# Resolve to wherever your persona file is installed — this repo's checkout,
# or the .github/agents/ directory of whatever repo the bundle was installed into.
