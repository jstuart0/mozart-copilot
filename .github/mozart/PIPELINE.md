...
**Tiers:** TINY (single reversible change — full loop, but skip otto's separate plan + xander/sebastian gate) / STANDARD (default) / HEAVY (storage, RBAC, secrets, live DB schema, production-stateful, resource-recreation — full pre-flight gate + user sign-off on irreversible steps).
