...
Subject: mozart's own field performance across consuming repos — the campaign artifacts (state files, flow sketches, plans, sebastian's reviews) are the evidence base. Deliverables: an eval report, configuration fixes, and an updated ledger. This is the institutional form of the kind of evaluation that produced fixes like the atomic-closeout and model-assignment repairs in the upstream editions: repeatable, delta-scoped, and — critically — able to answer "did the last round of fixes actually work?"

For the ledger schema and worked examples, see the reference doc `.github/mozart/EVAL.md` (a different file from this pipeline reference — that one is the schema; this one is the stage-by-stage procedure).

### Home and artifacts

EVAL spans projects, so its artifacts live in a **user-scope eval home** — not in any consuming repo, and not inside the installed bundle (which is meant to stay a faithful copy for `--check-install` to validate against). Resolve the eval home in this order: `$MOZART_EVAL_HOME` if set → `~/.mozart/evals/` (default; create with `mkdir -p` on first use).

- **Ledger**: `<eval-home>/ledger.jsonl` — append-only, machine-written, one record per (run, repo, slug, lens). Schema in `.github/mozart/EVAL.md`.
- **Report**: `<eval-home>/<YYYY-MM-DD>-eval.md` — metrics snapshot, findings, fixes shipped, and the next run's verification targets.
- **Fixes**: for mozart-copilot maintainers, normal commits to `.github/agents/`, `.github/mozart/`, `scripts/`, `CHANGELOG.md` in this repo. For consumers of an installed bundle who don't maintain this repo: project-level overrides (editing the installed `.github/agents/*.agent.md` directly in the consuming repo — a legitimate, deliberate divergence, the same one `CONTRIBUTING.md`'s local-install guidance describes for the Claude Code edition), field-note proposals, or an upstream PR to this repo. The report records which route each fix took.
...
2. **Mechanical metrics.** Run `scripts/mozart-lint.sh` per repo; snapshot the numbers into the report. Trends are the diff against the previous report's table. Also run `scripts/mozart-metrics.sh` per repo — it aggregates the campaigns' findings ledgers and escape links into the **pipeline-economics table**: confirmed catches by stage/lens/severity, false-positive rate per lens, escapes, defect-removal efficiency, and catches-per-campaign by tier (see `.github/mozart/EVAL.md` → Pipeline economics). These numbers are the evidence base for stage 5's gate-tuning decisions: a lens with zero catches and a high false-positive share over a meaningful sample gets its trigger tightened; a stage whose catches are all unique to it (nothing upstream found them) is earning its keep.
