# Flows: continuation, modes, build-time flags, partial flows, resume, run identification

## Continuing a dispatched agent vs dispatching fresh (detail)

The conductor body states the rule; this is the full discipline.

You have two ways to talk to a specialist:

- **Dispatch fresh (`agent` tool, new context)** — starts a *new* agent with an empty context. It re-reads the plan, the code, the prior findings from scratch. Use it for the **first** invocation of an agent in a stage, and any time you genuinely want a clean-slate perspective (e.g., an independent reviewer who shouldn't be anchored on a prior round's reasoning).
- **Continue the live agent** — sends a follow-up to an agent you **already dispatched this run**, by its name or returned ID, if the harness's continuation mechanism preserves context. Use it for **iteration on the same work** — feedback, punch-lists, "you missed X," "the user wants Y changed," answering a clarifying question an agent surfaced.

**Whether Copilot's continuation preserves context the way the plan it drafted, the diff it reviewed, and the reasoning it already did are all still loaded — is unverified for v1** (see `docs/COPILOT_PORT.md`, Known quirks and the campaign plan's open questions). Default to **brief-from-artifacts** as the primary, verified-working approach: re-dispatch the specialist and brief it with the durable artifacts (the plan file, sebastian's review, the punch-list, state-file notes) it needs to pick up where the prior round left off. If the harness's continuation mechanism is confirmed to preserve context in your environment, treat it as an available optimization for iteration loops — re-dispatching harry/jackson/valerie from scratch for a revision round throws away the exact context that makes the revision cheap and coherent — but never assume it works without confirming.

**Carve-out — a lens that supplied a constraint card.** A lens invoked at stage 2b or via a stage-3 consult (see harry's `## Consult requested` handling, stage 3) returns a constraint card, not a plan review — narrow, bounded, no artifact to review. When that same lens is invoked again at stage 4 to review the drafted plan, that is dispatched fresh, never continued — the default above does not apply here, and neither does the continuation discipline generally. The card and the stage-4 review are different work on different artifacts (a bound, then a judgment on a concrete plan), and continuing the live agent would anchor the stage-4 review on its own prior conclusion — the measured anchoring effect (arXiv 2603.12123; arXiv 2608.25869) that a fresh, unanchored dispatch avoids. Binds identically when a remediation entry (DIAGNOSE/AUDIT → DELIVER) runs 2b before stage 3. An adjudicating dick (see the conductor body's *The conductor record*) is always dispatched fresh, briefed with both claims and neither ranked.

**Resume caveat.** Live-agent continuity, where available, does not survive across mozart sessions. If you resume a campaign from a state file in a fresh session, the agents from the previous session are gone — there's nothing to continue. In that case, dispatch fresh and brief the new agent with the artifacts (plan file, sebastian's review, punch-list, state-file notes). The artifacts are the durable handoff; live agent context, where it exists at all, is a within-session optimization only.

**Narration.** A continuation is still a stage action — narrate it. Use the same `TASK [...]` cadence but make the verb explicit: `TASK [<slug>: iterate r1] Briefing harry with 3 reviewer findings (continuing — context intact)...` or `TASK [<slug>: iterate r1] Re-dispatching harry with 3 reviewer findings + the plan artifact (fresh context)...`.
...
When triggered:
- Run stages 1–6 as in STANDARD/HEAVY (intake → research → plan → reviewers → sebastian → iterate)
- Stop after stage 6 reaches convergence (no Critical/High findings remaining) or hits the iteration cap
- **Don't run jackson, mid-build specialists, sebastian's r2, valerie, or commit anything**
- Report cites: plan path, sebastian's r1 path, any open questions, and the iteration count
...
If the user later returns and says "implement that plan" / "go ahead and build it," re-enter at stage 7 using the existing plan + sebastian's r1 file. Don't re-run stages 2–6 unless the user wants the plan re-reviewed.
...
| "get a cross-model read on this plan" | **Stage 5 (Counterpoint on plan)** | Path to existing plan |
...
2. If `.mozart/plans/<slug>.counterpoint-r1-plan.md` exists, read it too — it tells you what was already addressed and what concerns survived review
...
5. Jump to stage 7. Stages 9–13 (sebastian on diff, validate, reconcile, documentation, report) run as usual, including 12b (Ship) when the repo's `## Pull requests` stanza enables it
...
- Run stage 4 (internal reviewers, conditional) + stage 5 (sebastian) + stage 6 (iterate if needed)
- Stop after stage 6
...
Every run that creates state files uses a naming convention that makes prior work searchable. The slug is the join key for all of a run's artifacts (state, flow sketch, plan, validation report, investigation, research brief).
...
- **Sortable by date** — listing `.mozart/plans/` shows chronological order
- **Filterable by shape** — a glob for `*-audit-*.md` finds every past audit; `*-diagnose-*` finds every investigation; `*-deliver-*` finds every feature/refactor delivery
- **Filterable by topic** — a glob for `*forgejo*` finds every run touching forgejo regardless of shape
- **Searchable across artifact types** — same slug for the plan, state, flow, validation, investigation, and research files, so all of a run's artifacts surface together

### At intake: discover prior art

Before locking the slug, search for prior runs that may inform this one. Two passes:

1. **Topic match** — substring-search the descriptive part against existing slugs. If "paperless" appears in any prior run, surface it. The user may want to extend it, supersede it, or just know what was tried before
