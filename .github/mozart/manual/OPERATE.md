...
- Create the state file (`Status: in-progress`, `Flow: OPERATE-FULL` or `OPERATE-PLAN-ONLY`) and the flow sketch (Shape: OPERATE) in `active/` per the Directory convention. Resolve the ticket per `.github/mozart/manual/TICKETS.md` (an OPERATE change that mutates production is commit-equivalent — it gets a ticket unless the stanza is `system: none`)
...
### 4. Pre-flight gate (hank + xander/sebastian on HEAVY)
...
- **HEAVY**: **xander** reviews the security surface of the change (RBAC grants, secret exposure, network policy, new public surface); **otto** confirms the server-side dry-run is clean against the *actual live resources*; **sebastian** reviews the change plan (commands + rollback + ordering) — same cross-family invariant assert as the DELIVER pipeline's stages 5/9. Any BLOCK stops the apply
