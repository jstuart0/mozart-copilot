# State persistence and the pipeline flow sketch
...
An enumerated list moves the extensions it names and strands every sibling it doesn't — the validation report, counterpoint review artifacts, test contracts. The glob catches everything the slug owns, which is the point of making the slug the join key. See the DELIVER pipeline's Campaign closeout section for the full transaction; this is the same command, repeated here because this is where the directory convention is defined.

When the campaign aborts, move to `aborted/` instead. The bare slug is the canonical identifier; tickets, commit messages, cross-references, and external links use the slug exactly. The directory is filesystem-only — it makes discovery cheap (listing `.mozart/plans/active/*.state.md`) without reading file contents.
...
**Source of truth is the `Status` field**, not the directory. If they ever drift (e.g., a crashed transition leaves `Status: complete` but the file still in `active/`), Status wins; a future mozart fixes the directory on next touch. The final-report-stage corruption check verifies the invariant.
...
- [ ] 9. Counterpoint on diff — <run|skip per tier>
- [ ] 10. Validate
- [ ] 11. Reconcile
...
|-------|-------|------|-----------------|----------------------|
| 5-counterpoint-r1 | sebastian | validation | <provider>/<model-id> | yes/no |
| 7-implement-p1 | jackson | builders | <provider>/<model-id> | yes/no |

One row per subagent dispatch this campaign. Populated from each response's `MODEL-ATTESTATION:` line — see D8/the cross-family invariant and each persona's Model attestation section. A `matched frontmatter?: no` row is a drift signal, not necessarily a failure; surface it, don't silently accept it.
...
4. **Alongside**: enter multi-campaign mode (see `.github/mozart/manual/WORKTREES.md`). Load the existing state/flow files, brief yourself on where each campaign stands, and add the new task as another concurrent campaign. Git isolation is normally already in place — each code-changing campaign cut its own worktree at intake. Verify that's true for every campaign you're about to run concurrently; for any that skipped one, confirm file-touch surfaces don't overlap before agreeing to parallel execution.
...
Wait for approval. On approval: commit, continue. On feedback: **brief the live jackson again** (context intact where the harness's continuation preserves it — he still has the phase diff loaded; brief-from-artifacts otherwise, see `.github/mozart/manual/FLOWS.md`) with the user's notes, re-run the gate, re-present. LOOP-IN does **not** replace the agent gates — it adds a user gate on top of them.
...
**Location**: `.mozart/plans/active/<slug>.flow.md` while active; `.mozart/plans/finished/<slug>.flow.md` once complete (alongside the plan and state files; see Directory convention above)
...
flowchart TD
    ...
    counterpoint1[Counterpoint r1 — sebastian]
    ...
    harry --> librarian
    bob --> counterpoint1
    librarian --> counterpoint1
    counterpoint1 --> jacksonP1
...
flowchart TD
    ...
    bob --> counterpoint1
    librarian --> counterpoint1
    dexter --> counterpoint1
    counterpoint1 --> jacksonP1 --> dick --> jacksonP2 --> ian --> valerie --> scott
    scott -.-> ship[12b Ship] -.-> report
    scott --> report
```
...
- **Stage 4** — added dexter (not in proposed flow). **Triggered by**: harry's plan introduced 3 new shared utilities; dexter pulled in for shallow-module review before sebastian
...
- **Track deviations as they happen.** Every divergence from proposed (added agent, skipped agent, re-run stage, escalated shape) goes into *Deviations from proposed* before you move on. Capture the *trigger* (the concrete reason — a sebastian finding, a regression, a scope change) not just the *what*. An empty Deviations section in a finalized run is a claim, not an absence — only use it when actual genuinely matched proposed.
