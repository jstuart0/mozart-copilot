# Worktree isolation and multi-campaign mode
...
- **Every agent brief carries two absolute paths and a branch**: the worktree path (where it works), the artifact path it reads or writes (`/abs/path/to/repo/.mozart/plans/active/<slug>.md`), and the campaign branch. An agent whose cwd is the worktree cannot resolve a relative `.mozart/...` path — it will silently create a stray one inside the worktree, where nothing will ever find it. Agents that *write* artifacts (harry's plan, tessa's test contract, dick's investigation) are the ones this bites; give them the absolute target, not a convention to re-derive. So is mozart when it persists on a grantless agent's behalf (sarah's returned brief, sebastian's returned findings — neither holds `edit` or `execute`).
- **sebastian is dispatched with the same absolute-path discipline** so its pre-computed input contract (`.mozart/plans/active/<slug>.counterpoint-r{1,2}-inputs/`) resolves regardless of its own cwd, and it's handed `git -C <worktree> diff <base>...HEAD` (pre-computed into `diff.patch` by mozart, since sebastian has no `execute`) rather than being asked to derive the diff itself.

Rules that make this safe rather than merely tidy:

- **Branch from the repo's documented base branch, not from whatever HEAD happens to be.** If `AGENTS.md` names a base (`main`, `develop`, `deploy/thor-staging`), use it. If the current branch is unexpected, surface it before cutting rather than silently inheriting it.
...
- **Worktrees isolate files, not runtimes.** Venvs, `node_modules`, ports, databases, and docker networks are still shared. See *Multi-campaign discipline* below — identity verification plus serialization, not per-worktree duplicate installs.
...
- Never force-remove a worktree to get past a dirty one. Dirty means uncommitted work exists; surface it.
...
### Parallelism within a single dispatch batch

The actual parallelism win is **batching agents across campaigns in a single message, multiple subagent dispatches**.

Example single-message batch:
- dispatch **bob** reviewing `feature-search`'s plan (worktree A)
- dispatch **harry** iterating `billing-refactor` plan from sebastian's r1 findings (worktree B)
- dispatch **jackson** implementing `perf-fix` phase 2 (worktree C)
...
- **Cap batch size.** 5–6 parallel dispatches is comfortable; beyond that, the user can't follow the narration and you risk context bloat. Roll into multiple batches if needed.

### Per-campaign narration

The live narration cadence stays (see `.github/mozart/manual/FLOWS.md` — no, see the conductor body's narration section for the full prefix format). The `TASK [...]` prefix on every line includes the campaign slug:

```
TASK [feature-search: Plan review] Dispatching bob, librarian, xander in parallel...
...
TASK [billing-refactor: Mid-build phase 2] Dispatching xander on phase 2 (HEAVY)...
```

For cross-campaign parallel batches, use `TASK [parallel batch]` and list each campaign's work in the body:

```
TASK [parallel batch] Dispatching: bob[feature-search: plan review], harry[billing-refactor: iterate r1], jackson[perf-fix: phase 2]
...
Each campaign maintains its own (see `.github/mozart/manual/FLOWS.md`, Run identification and prior-art discovery, for the slug format `<YYYY-MM-DD>-<shape>-<descriptive>`):
...
- **Checkpoint cleanly under context pressure.** If juggling N campaigns is filling your context faster than work is closing out, finalize state files for in-progress campaigns and surface: "Context is tight. Recommend resuming campaigns X, Y, Z in a fresh session — their state/flow files are up to date." Don't push until you blow the context.
